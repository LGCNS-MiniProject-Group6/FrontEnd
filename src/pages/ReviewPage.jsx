...
function ReviewPage() {
  ...
  // 현재는 공고와 검수 결과를 Mock 데이터로 보여줍니다.
        ...
        <div className="review-content">
          ...
          <section className="review-section">
            ...
            <div className="condition-list">
              {reviewMock.conditions.map((condition) => (
          ...
          <div className="review-notes-grid">
            <Card className="review-note review-note--warning">
              <h2>신청 전에 꼭 확인하세요</h2>
              <ul>{reviewMock.warnings.map((item) => <li key={item}>{item}</li>)}</ul>
            </Card>
            <Card className="review-note review-note--info">
              <h2>공고에서 확인된 준비서류</h2>
              <ul>{reviewMock.documents.map((item) => <li key={item}>{item}</li>)}</ul>
            </Card>
          </div>
          ...
          <div className="review-actions">
            ...
            <a className="button button--secondary button--large" href={program.originalUrl} target="_blank" rel="noreferrer">
              원문 공고 보기 ↗
            </a>
          </div>
        </div>
