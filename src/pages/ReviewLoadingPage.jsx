  ...
  return (
    <main className="loading-page">
      ...
      <section className="review-loading-card">
        ...
        <div className="review-loading-card__progress"><span style={{ width: `${((active + 1) / loadingSteps.length) * 100}%` }} /></div>
        <ol>{loadingSteps.map((item, index) => <li key={item} className={index < active ? 'done' : index === active ? 'active' : ''}><span>{index < active ? '✓' : index === active ? '●' : '○'}</span><div><strong>{item}</strong><small>{index < active ? '확인 완료' : index === active ? '현재 진행 중' : '다음 단계'}</small></div></li>)}</ol>
        <Button size="large" disabled={active < loadingSteps.length - 1} onClick={() => navigate(reviewPath(pblancId))}>{active < loadingSteps.length - 1 ? '검수 중입니다' : '검수 결과 보기'}</Button>
      </section>
    </main>
  )
}
