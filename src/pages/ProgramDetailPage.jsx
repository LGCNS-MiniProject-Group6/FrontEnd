  ...
  return (
    <AppLayout>
      ...
      <header className="detail-heading">
        <div><span className="eyebrow">{program.category}</span><h1>{program.title}</h1><p>{program.organization}</p><div className="badge-row"><StatusBadge tone="info">{program.category}</StatusBadge><StatusBadge tone="success">{getDday(program.applicationEndAt)}</StatusBadge></div></div>
        <Button variant="secondary">♡ 관심공고</Button>
      </header>
      <Card className="program-overview">
        <h2>공고 요약</h2>
        <dl><div><dt>신청기간</dt><dd>{formatPeriod(program.applicationStartAt, program.applicationEndAt)}</dd></div><div><dt>지원대상</dt><dd>{program.target}</dd></div><div><dt>지원내용</dt><dd>{program.summary}</dd></div><div><dt>신청방법</dt><dd>{program.method}</dd></div></dl>
      </Card>
